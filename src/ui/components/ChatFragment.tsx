...
export function ChatFragment({ username, ipaddr }) {
  ...
  const reconnect = async () => {
    notifier.notify({
      text: `Trying to reach ${ipaddr} in 10 seconds...`,
      type: "warning",
    });
  ...
  return (
    <View style={style.main}>
      ...
      <View
        ...
        onPointerDown={() => setPulv(false)}
        onTouchStart={() => setPulv(false)}
      >
        <Image
          style={style.chatImg}
          source={"chatroom.png"}
        ...
        <ScrollView
          ...
          contentContainerStyle={style.msgsContainer}
        >
          {messages.map((msg, i) => (
            <Text
              key={i}
      ...
      {pulv ? (
        <List.Section style={style.popUpView}>
          <List.Item
            ...
            title="Close Chat"
            left={() => (
              <List.Icon icon="close" color={style.closeChatBtn.color} />
            )}
          ...
          <List.Item
            ...
            left={() => (
              <List.Icon icon="delete" color={style.deleteContactBtn.color} />
            )}
