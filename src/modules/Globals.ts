...
export class Globals {
  ...
  private getGlobalObj(name: String) {
    const global = this.globals.find(
      (global) => global.name === name.toLowerCase(),
    );
    if (global === undefined) {
  ...
  updateGlobal(newGlobal: GlobalObj) {
    ...
    if (global.type === newGlobal.type) global.value = newGlobal.value;
    else {
  ...
  getGlobalsNames(): String[] {
    const names = [];
    for (let obj of this.globals) names.push(obj.name);
    return names;
  }
}
